...
class NoHardwarePumpController:
    ...
    def set_position_steps(self, pump_number: int, steps: int):
        ...
        if steps < 0:
            raise ValueError("position muss >= 0 sein")
...
class MIXmate:
    def __init__(self):
        ...
        self._auto_enable_relays()
        self.init_error = init_error

    def _auto_enable_relays(self):
        # Beim Start Relais aktivieren; falls gpiozero/GPIO nicht verfuegbar ist:
        # bewusst ignorieren, damit App normal weiter startet.
        try:
            self.admin_controller.relay_run_startup_sequence(delay_secs=0.5)
        except Exception as e:
            print(f"[Startup] Relais-Auto-Start uebersprungen: {e}")
