...
class AdminScreen(QWidget):
    ...
    def _build_setup_page(self) -> QWidget:
        ...
        relay_title = QLabel("Relais-Steuerung")
        relay_title.setObjectName("SectionTitle")
        c.addWidget(relay_title)

        relay_row_1 = QHBoxLayout()
        relay_row_1.setSpacing(10)
        btn_relay_1_on = QPushButton("Paar 1 EIN")
        btn_relay_1_off = QPushButton("Paar 1 AUS")
        btn_relay_2_on = QPushButton("Paar 2 EIN")
        btn_relay_2_off = QPushButton("Paar 2 AUS")
        for b in (btn_relay_1_on, btn_relay_1_off, btn_relay_2_on, btn_relay_2_off):
            b.setProperty("role", "nav")
            relay_row_1.addWidget(b)
        relay_row_1.addStretch(1)
        c.addLayout(relay_row_1)

        relay_row_2 = QHBoxLayout()
        relay_row_2.setSpacing(10)
        btn_relay_startup = QPushButton("Startsequenz")
        btn_relay_all_off = QPushButton("Alle AUS")
        btn_relay_refresh = QPushButton("Relais-Status")
        for b in (btn_relay_startup, btn_relay_all_off, btn_relay_refresh):
            b.setProperty("role", "nav")
            relay_row_2.addWidget(b)
        relay_row_2.addStretch(1)
        c.addLayout(relay_row_2)

        self.lbl_relay_state = QLabel("-")
        self.lbl_relay_state.setObjectName("InfoLabel")
        c.addWidget(self.lbl_relay_state)
        c.addStretch(1)
        ...
        btn_relay_refresh.clicked.connect(self.refresh_relay_state)
        btn_relay_1_on.clicked.connect(self.relay_pair_1_on)
        btn_relay_1_off.clicked.connect(self.relay_pair_1_off)
        btn_relay_2_on.clicked.connect(self.relay_pair_2_on)
        btn_relay_2_off.clicked.connect(self.relay_pair_2_off)
        btn_relay_startup.clicked.connect(self.relay_startup_sequence)
        btn_relay_all_off.clicked.connect(self.relay_all_off)
    ...
    def refresh_setup(self):
        try:
            ...
            levels = self.admin_controller.list_levels()
            relay_state = self.admin_controller.get_relay_state()
        except Exception as e:
            _msg_error(self, str(e))
            return
        ...
        relay_ready = bool(relay_state.get("ready", False))
        relay_pins = relay_state.get("pins", [])
        first_pair = relay_state.get("first_pair", [])
        second_pair = relay_state.get("second_pair", [])
        self.lbl_relay_state.setText(
            (
                f"Relais: {'READY' if relay_ready else 'NICHT INITIALISIERT'} | "
                f"Pins={relay_pins} | Paar1={first_pair} | Paar2={second_pair}"
            )
            if not relay_error
            else (
                f"Relais: FEHLER ({relay_error}) | "
                f"Pins={relay_pins} | Paar1={first_pair} | Paar2={second_pair}"
            )
        )
    ...
    def _run_relay_action(self, fn, ok_text: str):
        try:
            fn()
            self.refresh_relay_state()
            _msg_info(self, ok_text)
        except Exception as e:
            _msg_error(self, str(e))

    def relay_pair_1_on(self):
        self._run_relay_action(
            lambda: self.admin_controller.relay_set_pair(1, True),
            "Relais-Paar 1 eingeschaltet.",
        )

    def relay_pair_1_off(self):
        self._run_relay_action(
            lambda: self.admin_controller.relay_set_pair(1, False),
            "Relais-Paar 1 ausgeschaltet.",
        )

    def relay_pair_2_on(self):
        self._run_relay_action(
            lambda: self.admin_controller.relay_set_pair(2, True),
            "Relais-Paar 2 eingeschaltet.",
        )

    def relay_pair_2_off(self):
        self._run_relay_action(
            lambda: self.admin_controller.relay_set_pair(2, False),
            "Relais-Paar 2 ausgeschaltet.",
        )

    def relay_all_off(self):
        self._run_relay_action(
            self.admin_controller.relay_all_off,
            "Alle Relais ausgeschaltet.",
        )

    def relay_startup_sequence(self):
        self._run_relay_action(
            lambda: self.admin_controller.relay_run_startup_sequence(delay_secs=0.5),
            "Relais-Startsequenz abgeschlossen.",
        )
