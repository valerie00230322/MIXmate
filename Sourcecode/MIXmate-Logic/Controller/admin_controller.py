...
class AdminController:
    def __init__(self, db_path=None):
        # Modelle fuer DB-Zugriff und Maschinenparameter.
        # CocktailModel verwaltet Cocktail-Stammdaten und Rezepte.
        self.cocktail_model = CocktailModel(db_path=db_path)
        # IngredientModel verwaltet die Zutaten-Stammdaten.
        self.ingredient_model = IngredientModel(db_path=db_path)
        # PumpModel verwaltet Pumpenpositionen und Kalibrierwerte.
        self.pump_model = PumpModel(db_path=db_path)
        # LevelModel verwaltet die Regalebenen.
        self.level_model = LevelModel(db_path=db_path)
        # SystemSettingsModel speichert flexible Maschinenparameter.
    ...
    def get_level_height(self, levelnumber: int):
        # None bedeutet: fuer diese Ebene wurde noch kein Wert gesetzt.
        return self.settings_model.get_level_height(levelnumber)

    def set_level_direction(self, levelnumber: int, forward: bool):
        # Richtung wird als boolescher Maschinenparameter gespeichert.
    ...
    def set_cocktail_source_level(self, cocktail_id: int, levelnumber: int):
        # Cocktail merkt sich die Regalebene als Parameterzuordnung.
    ...
    def set_mixer_height(self, mixer_height_mm: float):
        # Mixerhoehe darf nicht unter den Nullpunkt fallen.
    ...
    def set_mixer_direction(self, left: bool):
        # UI-Auswahl links/rechts wird als bool gespeichert.
    ...
    def set_waiting_position(self, waiting_position_mm: float):
        # Warteposition liegt im positiven Fahrbereich.
    ...
    def set_waiting_direction(self, left: bool):
        self.settings_model.set_waiting_direction(bool(left))

    def get_waiting_direction(self):
        return self.settings_model.get_waiting_direction()

    def set_load_unload_position(self, position_mm: float):
        # Uebergabeposition liegt im positiven Fahrbereich.
    ...
    def set_load_unload_height(self, height_mm: float):
        if float(height_mm) < 0:
            raise ValueError("load_unload_height_mm muss >= 0 sein")
        self.settings_model.set_load_unload_height(float(height_mm))

    def get_load_unload_height(self):
        return self.settings_model.get_load_unload_height()

    def move_mixer_to_load_unload_height(self, height_mm: float):
        # Optionaler Engine-Hook fuer direkte Adminfahrten.
        if self.mix_engine:
            self.mix_engine.move_to_position(float(height_mm))
    ...
    def set_simulation_mode(self, enabled: bool):
        # Simulation schaltet Hardwareaktionen im Laptop-Modus um.
