...
from PySide6.QtWidgets import (
    ...
    QInputDialog, QDialog, QDialogButtonBox, QTextEdit, QComboBox, QDoubleSpinBox
)
...
class DistanceCalibrationDialog(QDialog):
    def __init__(self, parent: QWidget, title: str, options: list[dict]):
        super().__init__(parent)
        ...
        info = QLabel("Wähle die Distanz, fahre die Position an und speichere erst zum Schluss den finalen Wert.")
        ...
        target_label = QLabel("Distanz:")
        self.target_combo = QComboBox(self)
        for option in self.options:
            self.target_combo.addItem(option["label"])
        ...
        value_label = QLabel("Soll-Position:")
        self.position_spin = QDoubleSpinBox(self)
        self.position_spin.setRange(0.0, 99999.0)
        self.position_spin.setDecimals(2)
        self.position_spin.setSuffix(" mm")
        self.position_spin.setSingleStep(1.0)
        self.btn_drive = QPushButton("Auf Distanz fahren")
        self.btn_drive.setProperty("role", "nav")
        value_row.addWidget(value_label)
        value_row.addWidget(self.position_spin, 1)
        value_row.addWidget(self.btn_drive)
        root.addLayout(value_row)

        adjust_grid = QGridLayout()
        adjust_grid.setHorizontalSpacing(10)
        adjust_grid.setVerticalSpacing(10)
        self.adjust_buttons = []
        for index, delta in enumerate((5.0, 10.0, 20.0, -5.0, -10.0, -20.0)):
            label = f"{delta:+.0f} mm"
            btn = QPushButton(label)
            btn.setProperty("role", "nav")
            btn.clicked.connect(lambda _checked=False, step=delta: self._adjust_and_drive(step))
            adjust_grid.addWidget(btn, index // 3, index % 3)
            self.adjust_buttons.append(btn)
        root.addLayout(adjust_grid)

        self.current_value_label = QLabel("-")
        self.current_value_label.setObjectName("InfoLabel")
        ...
        self.target_combo.currentIndexChanged.connect(self._load_selected_value)
        self.btn_drive.clicked.connect(self._drive_to_current_value)
        buttons.accepted.connect(self._save_value)
        buttons.rejected.connect(self.reject)

        self._load_selected_value()

    def _current_option(self) -> dict:
        return self.options[self.target_combo.currentIndex()]

    def _load_selected_value(self):
        option = self._current_option()
        current = option["getter"]()
        if current is None:
            current = 0.0
        self.position_spin.setValue(float(current))
        self.current_value_label.setText(
            f"Aktueller DB-Wert: {float(current):.2f} mm"
        )

    def _drive_to_current_value(self):
        option = self._current_option()
        try:
            option["mover"](float(self.position_spin.value()))
        except Exception as e:
            _msg_error(self, str(e))

    def _adjust_and_drive(self, delta_mm: float):
        new_value = max(0.0, float(self.position_spin.value()) + float(delta_mm))
        self.position_spin.setValue(new_value)
        self._drive_to_current_value()

    def _save_value(self):
        option = self._current_option()
        try:
            option["setter"](float(self.position_spin.value()))
        except Exception as e:
            _msg_error(self, str(e))
            return
        self.accept()
...
class AdminScreen(QWidget):
    def __init__(self, admin_controller, pump_controller, mix_controller, on_back):
        super().__init__()
    ...
    def _build_mixer_settings_page(self) -> QWidget:
        ...
        self.lbl_waiting_direction = QLabel("-")
        ...
        btn_set_wait_direction.clicked.connect(self.set_waiting_direction)
        ...
        btn_calibrate.clicked.connect(self.open_mixer_distance_calibration)
    ...
    def _build_level_settings_page(self) -> QWidget:
        ...
        self.tbl_level_heights = QTableWidget(0, 3)
        self.tbl_level_heights.setHorizontalHeaderLabels(["levelnumber", "height_mm", "richtung"])
        ...
        btn_calibrate.clicked.connect(self.open_level_distance_calibration)
    ...
    def refresh_setup(self):
        try:
            ...
            waiting_direction = self.admin_controller.get_waiting_direction()
        ...
        self.lbl_waiting_direction.setText(
            f"Wartepositions-Richtung: {'links' if bool(waiting_direction) else 'rechts'}"
            if waiting_direction is not None
            else "Wartepositions-Richtung: -"
        )
        ...
        for lv in levels:
            ...
            self.tbl_level_heights.setItem(
                row, 2, QTableWidgetItem("links" if bool(level_direction) else "rechts")
            )
    ...
    def set_waiting_direction(self):
        current = self.admin_controller.get_waiting_direction()
        if current is None:
            current = True
        options = ["links", "rechts"]
        default_idx = 0 if bool(current) else 1
        ...
        if not ok:
            return
        try:
            self.admin_controller.set_waiting_direction(chosen == "links")
            self.refresh_setup()
        except Exception as e:
            _msg_error(self, str(e))
    ...
    def _open_distance_calibration_dialog(self, title: str, options: list[dict]):
        dlg = DistanceCalibrationDialog(self, title, options)
        if dlg.exec() == QDialog.Accepted:
            self.refresh_setup()
            _msg_info(self, "Distanz gespeichert.")

    def open_mixer_distance_calibration(self):
        options = [
            {
                "label": "Mixerhöhe",
                "getter": self.admin_controller.get_mixer_height,
                "setter": self.admin_controller.set_mixer_height,
                "mover": self.mix_controller.move_regal_lift_to_position,
            },
            {
                "label": "Warteposition",
                "getter": self.admin_controller.get_waiting_position,
                "setter": self.admin_controller.set_waiting_position,
                "mover": self.mix_controller.move_regal_lift_to_position,
            },
            {
                "label": "Sichere Homing-Höhe",
                "getter": self.admin_controller.get_homing_safe_height,
                "setter": self.admin_controller.set_homing_safe_height,
                "mover": self.mix_controller.move_regal_lift_to_position,
            },
            {
                "label": "Beladen/Entladen-Position",
                "getter": self.admin_controller.get_load_unload_position,
                "setter": self.admin_controller.set_load_unload_position,
                "mover": self.mix_controller.move_mixer_to_position,
            },
        ]
        self._open_distance_calibration_dialog("Mixer-Distanzen kalibrieren", options)

    def open_level_distance_calibration(self):
        ...
        options = [
            {
                "label": f"Ebenenhöhe Ebene {levelnumber}",
                "getter": lambda level=levelnumber: self.admin_controller.get_level_height(level),
                "setter": lambda value, level=levelnumber: self.admin_controller.set_level_height(level, value),
                "mover": self.mix_controller.move_regal_lift_to_position,
            },
        ]
        self._open_distance_calibration_dialog(
            f"Ebene {levelnumber} kalibrieren",
            options,
        )
    ...
    def add_level(self):
        if not _confirm(self, "Ebene hinzufügen", "Neue Ebene anlegen?"):
            return

        try:
            new_level = self.admin_controller.add_level()
            self.refresh_setup()
