...
class CalibrationRunWorker(QObject):
    ...
    def __init__(self, pump_controller, pump_number: int, seconds: int):
        super().__init__()
        # Primitive Werte vermeiden Qt-Thread-Zugriffe auf Widgets.
        self.pump_controller = pump_controller
        self.pump_number = int(pump_number)
        self.seconds = int(seconds)

    def run(self):
        # Pumpen-Kalibrierlauf im Hintergrund starten.
        try:
            actual_seconds = self.pump_controller.run_pump_for_calibration(
                self.pump_number,
                self.seconds,
            )
            # Tatsaechliche Laufzeit wird fuer Flow-Rate-Berechnung genutzt.
            self.finished.emit(int(actual_seconds))
        except Exception as e:
            # Fehlertext bleibt bewusst schlicht fuer QMessageBox/Overlay.
...
class CalibrationScreen(QWidget):
    def __init__(self, pump_controller, on_back):
        ...
        # Merkt die Pumpe fuer den Messwertdialog nach dem Lauf.
    ...
    def resizeEvent(self, event):
        # Overlay deckt die komplette Kalibrierseite ab.
        self.overlay.setGeometry(0, 0, self.width(), self.height())
        super().resizeEvent(event)

    def _go_back(self):
        # Laufende UI-Zustaende vor Navigation zuruecksetzen.
        self.stop()
        self.on_back()

    def start(self):
        # Pumpenliste beim Oeffnen aktualisieren.
        self.refresh()

    def stop(self):
        # Overlay beim Verlassen ausblenden.
        self._hide_overlay()

    def refresh(self):
        # Kalibrierliste immer aus aktuellen Pumpendaten bauen.
        self._clear_list()
        ...
        for p in pumps:
            # Jede Pumpe bekommt eine eigene Kalibrier-Aktion.
            self._add_pump_button(p)

    def _clear_list(self):
        # Alte Pumpenbuttons aus dem Layout entfernen.
