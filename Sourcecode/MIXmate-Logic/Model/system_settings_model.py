...
class SystemSettingsModel:
    def __init__(self, db_path=None):
        ...
        # Grundschema vor Parameterzugriff sicherstellen.
        ensure_database(db_path)
        self.db_path = db_path
        self.connection = sqlite3.connect(self.db_path)
        # Parameterwerte koennen per Spaltenname gelesen werden.
        self.connection.row_factory = sqlite3.Row
        self.cursor = self.connection.cursor()
        self._ensure_schema()

    def _ensure_schema(self):
        # Key-Value-Tabelle fuer Maschinenparameter.
        self.cursor.execute(
            """
    ...
    def set_value(self, key: str, value: float):
        # Leere Keys wuerden spaeter nicht mehr eindeutig wartbar sein.
        if not key:
            raise ValueError("param_key darf nicht leer sein")
        # Upsert haelt Speichern und Aktualisieren an einer Stelle.
        self.cursor.execute(
            """
    ...
    def get_value(self, key: str):
        # Nicht gesetzte Parameter werden als None zurueckgegeben.
        self.cursor.execute(
    ...
    def set_mixer_direction(self, left: bool):
        # SQLite speichert boolesche Werte als 1.0 oder 0.0.
    ...
    def get_mixer_direction(self):
        ...
        if val is None:
            # None zeigt eine noch nicht konfigurierte Richtung.
            return None
        return bool(int(val))
    ...
    def set_waiting_direction(self, left: bool):
        # Wartebandrichtung als numerischer Parameter.
        self.set_value("waiting_direction_left", 1.0 if bool(left) else 0.0)

    def get_waiting_direction(self):
        val = self.get_value("waiting_direction_left")
        if val is None:
            return None
        return bool(int(val))
    ...
    def set_load_unload_height(self, mm: float):
        self.set_value("load_unload_height_mm", mm)

    def get_load_unload_height(self):
        return self.get_value("load_unload_height_mm")
    ...
    def set_level_direction(self, levelnumber: int, forward: bool):
        # Jede Ebene bekommt einen eigenen Richtungs-Key.
    ...
    def set_level_height(self, levelnumber: int, mm: float):
        # Hoehe wird pro Regalebene gespeichert.
    ...
    def set_pump_distance(self, pump_number: int, distance_steps: float):
        # Pumpenposition bleibt historisch unter distance gespeichert.
    ...
    def set_cocktail_source_level(self, cocktail_id: int, levelnumber: int):
        # Rezeptquelle verbindet Cocktail und Regalebene.
    ...
    def get_cocktail_source_level(self, cocktail_id: int):
        ...
        if val is None:
            # Ohne Zuordnung greift spaeter der Fallback im Mixablauf.
            return None
        return int(val)

    def set_simulation_mode(self, enabled: bool):
        # Simulation wird als Maschinenparameter persistiert.
    ...
    def get_simulation_mode(self) -> bool:
        ...
        if val is None:
            # Produktivmodus bleibt Standard, solange nichts gesetzt ist.
            return False
        return bool(int(val))
